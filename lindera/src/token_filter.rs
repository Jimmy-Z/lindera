...
///   a `LinderaResult<()>`.
pub trait TokenFilter<T: Deref<Target = [u8]>>: 'static + Send + Sync + TokenFilterClone<T> {
...
/// static lifetime.
pub struct BoxTokenFilter<T: Deref<Target = [u8]>>(Box<dyn TokenFilter<T>>);

impl<T: Deref<Target = [u8]>> Deref for BoxTokenFilter<T> {
    type Target = dyn TokenFilter<T>;

    fn deref(&self) -> &dyn TokenFilter<T> {
        &*self.0
    }
}

impl<T: Deref<Target = [u8]>> BoxTokenFilter<T> {
    fn from(token_filter: impl TokenFilter<T>) -> BoxTokenFilter<T> {
...
pub trait TokenFilterClone<T: Deref<Target = [u8]>> {
    fn box_clone(&self) -> BoxTokenFilter<T>;
}

impl<T: Deref<Target = [u8]>, F: TokenFilter<T> + Clone> TokenFilterClone<T> for F {
    fn box_clone(&self) -> BoxTokenFilter<T> {
...
impl TokenFilterLoader {
    pub fn load_from_value<T>(kind: &str, value: &Value) -> LinderaResult<BoxTokenFilter<T>> where T: Deref<Target = [u8]> {
    ...
    pub fn load_from_cli_flag<T>(cli_flag: &str) -> LinderaResult<BoxTokenFilter<T>> where T: Deref<Target = [u8]> {
